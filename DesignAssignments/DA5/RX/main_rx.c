...
int main()
{
  ...
  ADC_init();
  SPI_init();
  nRF_init(RX);   // nRF as Receiver

  while (1)
  {
    uint8_t status;   
    uint16_t value = ReadADC(8);                        // read internal temperature
    nRF_sendData(W_ACK_PAYLOAD, (uint8_t *)&value, 2);  // send the value to FIFO to be sent along with acknowledgement  
